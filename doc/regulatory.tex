...
\srcref[artikel=3.126, lid=1, onderdeel={c,d}]{wetib}
    artikel 3.126, eerste lid, onderdelen c en d, van de Wet IB 2001
\srcref[artikel={162--164}]{bw6}
...
\translation{%
    ...
    already: \texttt{dutch} for a Dutch regulation, and \texttt{dutch\_eu}, \texttt{german\_eu},
    \texttt{french\_eu} and \texttt{english\_eu} for an act of the Union cited in Dutch, German, French or
    English. A register is not a language: the same
...
\translation{%
    The definition files of the four languages this bundle ships follow.
    They are the source a translation starts from, so they are printed here rather than among the modules
    of \zcref{sec:impl}.
    Two of them are half a language: German and French carry the designations and the citation register
    that were measured, and not the wording of an internal reference, which was not.%
}{%
    ...
    Hieronder volgen de definitiebestanden van de vier meegeleverde talen.
    ...
    Twee ervan zijn half: het Duits en het Frans dragen de benamingen en het citeerregister die gemeten
    zijn, en niet de verwoording van een interne verwijzing, die dat niet is.%
}
...
\translation{%
    ...
    Dat er een document uitkomt is de kleinste helft van wat een run vaststelt. Vrijwel alles
    ...
    label dat nergens heen wees. Iedere suite eindigt daarom in een reeks controles die de run
...
\translation{%
    Six documents are typeset, of which two exist in both languages, so a suite makes eight of
    them. Every one of them is a driver that sets the language, declares the artifacts and loads
    the definitions, and that leaves the body to a file of its own, so that both language
    versions share one text:%
}{%
    Er worden zes documenten gezet, waarvan twee in beide talen bestaan, zodat een suite er
    acht maakt. Ieder daarvan is een driver die de taal instelt, de artefacten declareert en de
    definities inlaadt, en die de inhoud overlaat aan een eigen bestand, zodat beide taalversies
    één tekst delen:%
}
\begin{labeling}{\file{example1-nl.tex}}
    ...
    \item[sign-example.tex] \translation{%
        ...
        PDF file without a PDF tool.%
    }{%
        welke de handtekeningvelden van \zcref{sec:sign} plaatst en helemaal geen definities
        heeft. Het wordt ongecomprimeerd geschreven, zodat de annotatie over een veld zonder
        PDF-gereedschap uit het PDF-bestand terug te lezen is.%
    }
    \item[source-example.tex] \translation{%
        ...
        welke \file{sources.bib} inlaadt en aanhaalt wat daarin staat, in ieder register van
...
\translation{%
    ...
    at each other, which is what \zcref{sec:extern} is about. The other two stand on their own
    and are drawn nowhere below.
    ...
    ze naar elkaar wijzen, waar \zcref{sec:extern} over gaat. De andere twee staan op zichzelf en
    zijn hieronder niet getekend.
...
\translation{%
    Every engine suite ends in seven sets of assertions, each of which prints what it counted and
    ...
    Iedere engine-suite eindigt in zeven reeksen controles, die elk opschrijven wat ze geteld
    ...
    zijn taak is om diezelfde stilte de volgende keer hoorbaar te maken. Alle zeven zijn expres
...
\begin{labeling}{\texttt{source-checks}}
    ...
    \item[source-checks] \translation{%
        ...
        thirty-eight citations word for word. A citation in the wrong register fails nowhere; it
        ...
        formulering van achtendertig aanhalingen woord voor woord. Een aanhaling in het verkeerde
    ...
    \item[html-checks] \translation{%
        ...
        without an error, since a lost configuration produces plain text and not a failure.%
    }{%
        ...
        want een verloren configuratie levert platte tekst op en geen fout.%
    }
\end{labeling}
...
\translation{%
    ...
    That difference is measured rather than read out of a standard: the same document with the
    ...
    Dat verschil is gemeten en niet uit een standaard gelezen: hetzelfde document met dezelfde
