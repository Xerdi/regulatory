...
\translation{%
    ...
    Er is ook een HTML-conversie, welke op \texttt{make4ht} draait en een eigen configuratie nodig heeft;
    zie \zcref{sec:html}.%
}
...
\begin{labeling}{\package{regulatory-attachments}}
    ...
    \item[regulatory-attachments] \translation{%
        ...
        de interdocumentaire verwijzingen en PDF-bijlagen van \zcref{sec:extern}, welke op zijn beurt
...
\translation{%
    ...
    De HTML-conversie is geen module maar een \package{tex4ht} configuratie, \file{regulatory.4ht}, welke
...
\translation{%
    ...
    Iedere optie is een sleutel van de \texttt{/regulatory} familie, welke
    \package{regulatory-struct} voor de gehele bundel administreert.
...
\begin{labeling}{\option{sourcestyle}\quad\texttt{alttree}}
    \addtokomafont{labelinglabel}{\ttfamily}
    \item[bib2gls\quad true] \translation{%
    ...
    \item[alldefs\quad false] \translation{%
    ...
    \item[legacy\quad false] \translation{%
    ...
    \item[nameinlink\quad true] \translation{%
    ...
    \item[defstyle\quad alttree] \translation{%
    ...
    \item[sourcestyle\quad full] \translation{%
    ...
    \item[attachmentlink\quad goto] \translation{%
        how an attachment of \zcref{sec:extern} is opened from the page. With \texttt{goto} the link
        is an embedded go-to action, which names the file inside this document and is the right thing
        to say\,---\,and is the one action \package{poppler} does not implement, so under most
        viewers on a free desktop the link does nothing. With \option{attachmentlink=annotation} a
        file attachment annotation is placed over the same text instead, which Acrobat and
        \package{poppler} both do implement. It shows nothing of its own: the text stays the only
        thing on the page, and it is the annotation that opens. Either way the file is also in the
        attachment pane of the viewer, which needs neither.

        A file attachment is a markup annotation, and PDF/UA asks that one be enclosed in an
        \texttt{Annot} structure element; this route puts it there, as the reference beside it sits
        in its \texttt{Link} element. Do not take a validator's word for that either way: the
        veraPDF this manual is measured with reported nothing when the element was missing and an
        older one did, so the suite reads the structure tree out of the PDF itself rather than
        asking for a verdict.

        What remains between the two is a question about documents and not about PDF. The
        \texttt{goto} route makes the text of \cmd{\documentattachment} a link in the sentence it
        stands in; this one puts an annotation beside it and leaves the text as text. A reference a
        reader meets in the flow of a sentence is not the same offer as one beside it, which is why
        \texttt{goto} is the default.%
    }{%
        hoe een bijlage uit \zcref{sec:extern} vanaf de pagina geopend wordt. Met \texttt{goto} is de
        link een embedded go-to action, die het bestand binnen dit document noemt en daarmee zegt wat
        er te zeggen valt\,---\,en is tegelijk de ene actie die \package{poppler} niet
        implementeert, zodat de link onder de meeste viewers op een vrij bureaublad niets doet. Met
        \option{attachmentlink=annotation} komt er in plaats daarvan een file attachment annotatie
        over dezelfde tekst, die Acrobat en \package{poppler} allebei wel kennen. Hij laat zelf niets
        zien: de tekst blijft het enige op de pagina en de annotatie is wat opent. Hoe dan ook staat
        het bestand ook in het bijlagenpaneel van de viewer, dat geen van beide nodig heeft.

        Een bijlage-annotatie is een markup-annotatie, en PDF/UA vraagt dat zo een in een
        \texttt{Annot}-structuurelement zit; deze route zet hem daarin, zoals de verwijzing ernaast
        in haar \texttt{Link}-element staat. Neem een validator daarin niet op zijn woord, welke
        kant ook op: de veraPDF waarmee deze handleiding gemeten is meldde niets toen het element
        ontbrak en een oudere meldde het wel, dus de suite leest de structuurboom zelf uit de PDF in
        plaats van om een oordeel te vragen.

        Wat er tussen de twee overblijft is een vraag over documenten en niet over PDF. De route
        \texttt{goto} maakt van de tekst van \cmd{\documentattachment} een link in de zin waar hij
        staat; deze zet er een annotatie naast en laat de tekst tekst. Een verwijzing die een lezer
        in de loop van een zin tegenkomt is een ander aanbod dan een die ernaast staat, en daarom is
        \texttt{goto} de standaard.%
    }
\end{labeling}
\translation{%
...
\translation{%
    ...
    De \texttt{paras} environment bevat de twee niveaus onder een artikel, gebouwd met \package{enumitem}.
...
\translation{%
    That \cmd{\abalphnum} comes from \package{fmtcount}\footnote{%
        The Dutch language definition is now included in \href{https://github.com/Xerdi/fmtcount}{\package{fmtcount}}
        itself. Be sure to update the package if \texttt{fc-dutch.def} is missing.%
    ...
    Die \cmd{\abalphnum} komt uit \package{fmtcount}\footnote{%
        ...
        Zorg ervoor dat het pakket up-to-date is als \texttt{fc-dutch.def} mist.%
...
\translation{%
    ...
    De \texttt{provisions} environment is een vlakke opsomming, genummerd binnen de \cmd{\section}
...
\translation{%
    ...
    \zcref{sec:conformance} en slaagt; de harnas maakt van een onverwacht geslaagd geval een fout, en zo
...
\translation{%
    ...
    that reason. This bundle does not print the date and does not build the link yet, but it does not let a
    document leave it unsaid, because the alternative is a citation that silently means \enquote{whenever
    this was last typeset}. \cmd{\sourcedate} says it for the whole document; one citation may say its own
    with \texttt{date=\meta{YYYY-MM-DD}}, which also answers to \texttt{datum}.
    ...
    de verwijzing zelf. Deze bundel drukt de datum niet af en bouwt de link nog niet, maar laat een document
    hem niet ongezegd, want het alternatief is een aanhaling die stilzwijgend \enquote{wanneer dit voor het
    laatst gezet is} betekent. \cmd{\sourcedate} zegt het voor het hele document; één aanhaling mag zijn
    eigen datum geven met \texttt{date=\meta{JJJJ-MM-DD}}, wat ook naar \texttt{datum} luistert.
...
\translation{%
    ...
    bepaling\,---\,\texttt{article}, \texttt{paragraph}, \texttt{point} en \texttt{subpoint}, welke ook naar
...
\translation{%
    Declares a register: eight keys for the full form and eight for the shortened one. Five are declared
    ...
    Declareert een register: acht sleutels voor de voluit-vorm en acht voor de verkorte. Er zijn er vijf
...
\translation{%
    Nine documents are typeset, of which two exist in both languages, so a suite makes ten of
    them per engine and four in the conversion of \zcref{sec:html}. One of the nine is converted
    only, since what it is for is only read under \package{tex4ht}.
    ...
    Er worden negen documenten gezet, waarvan twee in beide talen bestaan, zodat een suite er
    tien per engine maakt en vier in de omzetting van \zcref{sec:html}. Eén van de negen wordt
    alleen omgezet, want waar het voor is wordt alleen onder \package{tex4ht} gelezen. Ieder daarvan is een driver die de taal instelt, de artefacten declareert en de
...
\begin{labeling}{\file{example1-nl.tex}}
    ...
    \item[index-example.tex] \translation{%
        ...
        welke de tweede definitieroute van \zcref{sec:defs} neemt: \option{bib2gls=false}, zodat
        ...
        \texttt{bib2gls} tussen de runs nodig heeft, en tot het geschreven werd had die route geen
        enkele dekking.%
    }
    \item[nolang-example.tex] \translation{%
        ...
        welke helemaal geen taal noemt en omgezet wordt in plaats van gezet. Ieder ander document
    ...
    \item[date-example.tex] \translation{%
        ...
        welke aanhaalt zonder te zeggen per wanneer, en daarna een bron aanhaalt waarvan de
...
\translation{%
    Every engine suite ends in nine sets of assertions, each of which prints what it counted and
    ...
    Iedere engine-suite eindigt in negen reeksen controles, die elk opschrijven wat ze geteld
    ...
    zijn taak is om diezelfde stilte de volgende keer hoorbaar te maken. Alle negen zijn expres
...
\begin{labeling}{\texttt{source-checks}}
    ...
    \item[index-checks] \translation{%
        ...
        de runs iets gedaan heeft. De woordenlijstbestanden worden vóór de run weggegooid: gemeten,
...
\translation{%
    ...
    Eén geval staat op \texttt{xfail}. Het faalt op regel 6.8-4 van ISO 19005-3, op een bestandsspecificatie voor
