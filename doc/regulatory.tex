...
\translation{%
    ...
    When defining a new language, it's important to know that the default values are based on the English
    configuration, which is why \texttt{regulatory-english.def} passes no options at all.%
}{%
    Deze opties kunnen verschillen per artikel, lid en onderdeel.
    Bij het definiëren van een nieuwe taal is het belangrijk om te weten dat de standaard waarden gebaseerd
    zijn op de Engelse configuratie, wat de reden is dat \texttt{regulatory-english.def} in het geheel geen
    opties meegeeft.%
}
