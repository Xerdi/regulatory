...
\noindent
\DescribeMacro{\newsourcetype\\\marg{type}\\\marg{required fields}}
\translation{%
    Declares a type and the fields an entry of it cannot do without. Two are declared already:
    \texttt{regeling}, which needs a \texttt{citeertitel}, and \texttt{euhandeling}, which needs a
    \texttt{soort} and a \texttt{nummer}. A source that misses one of them stops the run, which is what
    lets everything downstream read a field without asking whether it is there.%
}{%
    Declareert een type en de velden waar een entry van dat type niet zonder kan. Er zijn er twee
    gedeclareerd: \texttt{regeling}, dat een \texttt{citeertitel} nodig heeft, en \texttt{euhandeling},
    dat een \texttt{soort} en een \texttt{nummer} nodig heeft. Een bron die er één mist stopt de run, en
...
\translation{%
    Declares a register: seven keys for the full form and seven for the shortened one. Two are declared
    already, \texttt{nl} and \texttt{nl\_eu}. A register is not a language: the same document may cite in
    both, and it does so the moment it cites a Dutch statute next to a regulation of the Union.%
}{%
    Declareert een register: zeven sleutels voor de voluit-vorm en zeven voor de verkorte. Er zijn er twee
    gedeclareerd, \texttt{nl} en \texttt{nl\_eu}. Een register is geen taal: hetzelfde document kan in
    beide aanhalen, en doet dat zodra het naast een Nederlandse wet een verordening van de Unie noemt.%
}
\clearpage
