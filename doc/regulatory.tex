...
\translation{%
    ...
    option \option{md} comes down to loading \package{markdown} with the options this bundle expects.
    ...
    With it, this package ensures that all these structures are handled.
    ...
    Refer to \zcref{code:example-md} for a Markdown example, and check \zcref{code:md-example} to see how such a Markdown source can ultimately be used from within \LaTeX{} (\zcref{sec:example}).
}{%
    ...
    pakketoptie \option{md} komt neer op het laden van \package{markdown} met de opties die deze bundel
    verwacht.
    ...
    Daarmee zorgt dit pakket ervoor dat al deze structuren gehanteerd worden.
    ...
    Kijk naar \zcref{code:example-md} voor een markdown voorbeeld en naar \zcref{code:md-example} hoe zo'n Markdown bron uiteindelijk kan gebruikt worden vanuit \LaTeX{} (\zcref{sec:example}).
}
\clearpage
...
\noindent
\translation{%
    The two \texttt{xfail} cases fail ISO 19005-3 rule 6.8-4 on a file specification for the
    external document: it carries \texttt{/AFRelationship /Unspecified}, has no \texttt{/EF} and
    is not listed in \texttt{/AF}. It is written for the hyperlink to that document, not by this
    package, so PDF/A-3 stays out of reach until that is settled upstream.%
}{%
    De twee \texttt{xfail} gevallen falen op regel 6.8-4 van ISO 19005-3, op een
    bestandsspecificatie voor het externe document: die draagt
    \texttt{/AFRelationship /Unspecified}, heeft geen \texttt{/EF} en staat niet in \texttt{/AF}.
    Hij wordt geschreven voor de hyperlink naar dat document en niet door dit pakket, dus
    PDF/A-3 blijft buiten bereik tot dat verderop opgelost is.%
}
\clearpage
