...
%    B\ij{}voorbeeld \cmd|\zref{sub:eerste}| resulteert in \zcref{ex1-lid:lorem}, in plaats van \aref{ex1-lid:lorem}.\\
%
%    Deze macro's zijn geschreven met behulp van \package{zref} en \package{xassoccnt}.
%    \cmd|\Aref| kijkt naar het eerste label en genereert daarna het voorvoegsel, b\ij{}voorbeeld voor een lid is het voorvoegsel `artikel 1,~'.
...
%    Neem b\ij{}voorbeeld \cmd|\aref{lid:lorem,sub:lorem}|, wat in dit geval leidt tot een foutmelding.
...
%    Stel dat `onderdeel a' een andere aanduiding dient te hebben, b\ij{}voorbeeld `sub.\ a'.
...
%    \clearpage
    ...
    \DescribeMacro{\reffootnotelabel}
    \DescribeMacro{\deffootnotelabel}
    \texttt{[WIP]}\footnote{Deze macro's zijn nog in ontwikkeling. De kans is groot dat er nog wijzigingen in aangebracht worden.}
    Voor zowel verwijzingen naar definities als artikelen e.d.\ wordt bij de eerste verschijning een voetnoot geplaatst.
    Onder water wordt \cmd|\reffootnotelabel|\marg{label} aangeroepen voor artikelen e.d.\ en voor definities \cmd|\deffootnotelabel|\marg{label}.\\

    Deze macro's kunnen ook handmatig uitgevoerd worden.
    Neem b\ij{}voorbeeld \cmd|\aref{ex1-sub:lorem}\reffootnotelabel{example1}| wat zou leiden tot:
    `\aref{ex1-sub:lorem}\reffootnotelabel{example1}'.
    En voor \cmd|\gls{def4}\deffootnotelabel{example2}| is de uitkomst `\gls{def4}'.
